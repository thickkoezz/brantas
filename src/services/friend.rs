...
use crate::db::DB;
use crate::dtos::friend::FriendDTO;
...
pub async fn add_friend(req: FriendDTO) -> AppResult<FriendDTO> {
  let db = DB
    .get()
...
pub async fn get_friend(paginator_option: Option<PaginatorOption>) -> AppResult<Vec<FriendDTO>> {
  let db = DB
    .get()
    .ok_or(anyhow::anyhow!(t!("database_connection_failed")))?;
  match paginator_option {
    Some(paginator_option) => {
      ...
      let res = friends
        .into_iter()
        .map(|friend: friend::Model| FriendDTO::from(friend))
        .collect::<Vec<_>>();
      Ok(res)
    }
    None => {
