...
pub async fn add_chat_group(req: ChatGroupDTO) -> AppResult<ChatGroupDTO> {
  let db = DB
    .get()
...
pub async fn get_chat_group(
  ...
  match paginator_option {
    Some(paginator_option) => {
      ...
      let res = chat_groups
        .into_iter()
        .map(|chat_group: chat_group::Model| ChatGroupDTO::from(chat_group))
        .collect::<Vec<_>>();
      Ok(res)
    }
    None => {
