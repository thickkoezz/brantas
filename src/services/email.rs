...
use crate::db::DB;
use crate::dtos::email::{EmailAddRequest, EmailResponse};
use crate::entities::{email, prelude::Email};
use sea_orm::prelude::DateTimeWithTimeZone;
use sea_orm::sqlx::types::chrono;
use sea_orm::{ActiveModelTrait, EntityTrait, PaginatorTrait, Set};

pub async fn add_email(req: EmailAddRequest) -> AppResult<EmailResponse> {
  let db = DB
    .get()
...
pub async fn get_email(paginator_option: Option<PaginatorOption>) -> AppResult<Vec<EmailResponse>> {
  let db = DB
    .get()
    .ok_or(anyhow::anyhow!(t!("database_connection_failed")))?;
  match paginator_option {
    Some(paginator_option) => {
      ...
      let res = emails
        .into_iter()
        .map(|email: email::Model| EmailResponse::from(email))
        .collect::<Vec<_>>();
      Ok(res)
    },
    None => {
